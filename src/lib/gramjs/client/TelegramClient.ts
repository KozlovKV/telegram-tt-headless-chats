...
import type { Connection, UpdateServerTimeOffset } from '../network';
import type { PasswordResult, TmpPasswordResult, TwoFaParams } from './2fa';
import type { UserAuthParams } from './auth';
import type {
  DownloadFileParams,
  DownloadFileWithDcParams,
  DownloadMediaParams,
} from './downloadFile';
...
import { Api } from '../tl';
import { getCurrentPassword, getTmpPassword, updateTwoFaSettings } from './2fa';
...
class TelegramClient {
  ...
  private _exportedSenderPromises: Record<
    number,
    Record<number, Promise<MTProtoSender> | undefined>
  > = {};
  ...
  private _exportedSenderReleaseTimeouts: Record<
    number,
    Record<number, TimeoutId | undefined>
  > = {};
  ...
  async connect() {
    ...
    await this._sender.send(this._initWith(new Api.help.GetConfig()));
    ...
    const mediaSender = await this._borrowExportedSender(
      this.session.dcId,
      false,
      undefined,
      0,
  ...
  async _initSession() {
    ...
    if (
      ...
      this.session.setDC(
        ...
        this._args.isTestServerRequested,
      );
    }
  }
  ...
  disconnect() {
    ...
    Object.values(this._exportedSenderPromises).forEach((promises) => {
  ...
  async _switchDC(newDc: number) {
    ...
    const isTestServer =
      this.session.isTestServer || this._args.isTestServerRequested;
  ...
  downloadMedia(
    ...
    let media;
    if (
      entityOrMedia instanceof Api.Message ||
      entityOrMedia instanceof Api.StoryItem
    ) {
  ...
  pickFileSize(
    sizes: (Api.TypePhotoSize | Api.TypeVideoSize)[],
    sizeType?: SizeType,
  ) {
  ...
  _downloadPhoto(
    media: Api.MessageMediaPhoto | Api.TypePhoto,
    args: DownloadMediaParams,
  ) {
    let photo = media;
    if (
    ...
    const size = this.pickFileSize(
      [...videoSizes, ...photo.sizes],
      args.sizeType,
    );
  ...
  _downloadDocument(
    media: Api.MessageMediaDocument | Api.TypeDocument,
    args: DownloadMediaParams & DownloadFileParams,
  ) {
    let doc = media;
    if (
      doc instanceof Api.MessageMediaDocument &&
      doc.document instanceof Api.Document
    ) {
      doc = doc.document;
    }
    ...
    let size;
    if (args.sizeType) {
      size = this.pickFileSize(
        [...(doc.thumbs || []), ...(doc.videoThumbs || [])],
        args.sizeType,
      );
  ...
  async _downloadWebDocument(media: Api.TypeWebDocument) {
    ...
    try {
      ...
      if (
        ...
        err.errorMessage === 'WEBFILE_NOT_AVAILABLE'
      ) {
    ...
    for (attempt = 0; attempt < this._requestRetries; attempt++) {
      ...
      try {
        ...
        if (
          ...
          const shouldRaise =
            e instanceof PhoneMigrateError || e instanceof NetworkMigrateError;
          ...
          if (isExported) this.releaseExportedSender(sender);
          sender = (
            dcId === undefined ? this._sender : await this.getSender(dcId)
          )!;
          ...
          e instanceof RPCError &&
          e.errorMessage === 'CONNECTION_NOT_INITED'
        ) {
          this.disconnect();
          await sleep(2000);
          await this.connect();
  ...
  getCurrentPassword(
    currentPassword?: string,
  ): Promise<PasswordResult | undefined> {
  ...
  _processUpdate(
    update: Update,
    entities: (Api.TypeUser | Api.TypeChat)[] | undefined,
  ) {
  ...
  async _dispatchUpdate(args: { update: Update }) {
